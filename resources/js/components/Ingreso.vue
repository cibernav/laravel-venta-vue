                ...
                <div class="card-body">
                    <div class="form-group row border">
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <em class="invalid-feedback error-text" v-show="errorLista.idproveedor">{{ errorLista.idproveedor ? errorLista.idproveedor[0] : '' }}</em>
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <table class="table table-bordered table-striped table-sm">
                                ...
                                <tbody v-if="form.arrayItems.length">
                                    <tr v-for="(item,index) in form.arrayItems" :key="item.idarticulo">
                                        ...
                                        <td>
                                            <input type="number" value="2" v-model="item.precio" class="form-control">
                                        </td>
                                        <td>
                                            <input type="number" value="2" step="any" v-model="item.cantidad" class="form-control">
                                        </td>
                                        <td class="text-right">
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            ...
                            <button type="button" class="btn btn-primary" @click="registrarModel()">Registrar ingreso</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    computed: {
        ...
        calcularTotal: function(){
            ...
            for (let i = 0; i < this.form.arrayItems.length; i++) {
                resultado += (this.form.arrayItems[i].precio * this.form.arrayItems[i].cantidad);
            }
            return resultado;
        }
    },
    methods: {
        listarData: function(page = 1, buscar = '', criterio = ''){
            let self = this;
            axios.get('/api/ingreso?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio).then(function (response){
        ...
        selectProveedor: function(search, loading){
            let self = this;
            loading(true);
            axios.get('/api/proveedor/selectactivo?filtro=' + search)
            .then(function (response){
                var respuesta = response.data;
                //q:search
                self.arrayProveedor = respuesta.data;
                loading(false);
            })
            .catch(function(error){
                console.log(error);
        ...
        getDatosProveedor: function(valselect){
            let self = this;
            self.loading = true;
            self.form.idproveedor = valselect != null ? valselect.id : 0;
        },
        buscarArticulo:function(){
            let me = this;
            axios.get('/api/articulo/buscar?filtro=' + this.detail.codigo)
            .then(function (response){
                var respuesta = response.data;
        ...
        listarArticuloModal: function(buscar = '', criterio = ''){
            let me = this;
            axios.get('/api/articulo/listar?buscar=' + buscar + '&criterio=' + criterio)
            .then(function (response){
                var respuesta = response.data;
        ...
        desactiveModel: function(id){
            ...
            reverseButtons: true
                ...
                if (result.value) {
                    let self = this;
                    axios.patch('/api/ingreso/' + id + '/desactive')
                    .then(function(response){
                        self.listarData();
                        swal.fire(
                        'Anulado!',
        ...
        resetModel: function(data = null, items = null){
            if(data == null){
                ...
                //this.form.idproveedor = data.idproveedor,
